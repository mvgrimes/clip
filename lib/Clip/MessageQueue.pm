...
sub _build_mq {
    ...
    $log->debugf("[CMQ] creating AnyEvent::RabbitmQ");
...
sub connect {
    ...
    $log->debugf("[CMQ] connecting to MQ");
    ...
    if ( not $self->mq->is_open ) {
        $self->mq->load_xml_spec->connect(
            ...
            ## exhange    => 'foo',
            on_success => sub {
                $log->debugf("[CMQ] connected");
                $d->resolve;
            },
            on_failure => sub {
                $log->warnf("[CMQ] connect failed: %s", p @_);
                $d->reject;
            },
        );
    } else {
...
sub open_channel {
    ...
    $log->debugf("[CMQ] creating channel");
    ...
    if ( ! $self->channel or ! $self->channel->is_open ) {
        $self->mq->open_channel(
            on_success => sub {
                my $channel = shift;
                $log->debugf("[CMQ] open_channel succeeded");
                $self->channel($channel);
                $d->resolve;
            },
            on_failure => sub {
                $log->warnf("[CMQ] open_channel failed");
...
sub declare_exchange {
    ...
    $log->debugf("[CMQ] delaring exchange");
    ...
    if ( not $self->exchange_delared ) {
        ...
        $self->channel->declare_exchange(
            ...
            type       => 'topic',
            on_success => sub {
                $log->debugf("[CMQ] declare_exchange succeeded");
                $self->exchange_delared(1);
                $d->resolve;
            },
            on_failure => sub {
                $log->warnf("[CMQ] declare_exchange failed");
...
sub _publish {
    ...
    $log->debugf("[CMQ] sending message: $msg");

    my $topic_str = sprintf "%s.%s", 'cmq', ( $topic // 'general' );
...
sub declare_queue {
    ...
    $self->channel->declare_queue(
        ...
        on_success  => sub {
            ...
            $log->debugf("[CMQ] declare_queue succeeded: $queue");
            $d->resolve($queue);
        },
        on_failure => sub {
            $log->warnf("[CMQ] declare_queue failed");
            die "Unable to open channel";
            $d->reject;
...
sub bind_queue_to_exchange {
    ...
    my $topic_str = sprintf "%s.%s", 'cmq', ( $topic // '#' );
    $log->debugf( "[CMQ] binding queue %s to exchange (%s) w/ topic %s",
    ...
    $self->channel->bind_queue(
        ...
        routing_key => $topic_str,
        on_success  => sub {
            $log->debugf("[CMQ] bind_queue succeeded: $queue");
            $d->resolve( $queue );
        },
        on_failure => sub {
            $log->warnf("[CMQ] bind_queue failed");
...
sub _create_consumer {
    ...
    $self->channel->consume(
        ...
        on_consume => sub {
            ...
            $log->debugf( "[CMQ] receved msg: " . $body );
...
sub subscribe {
    ...
    $log->debugf("[CMQ] creating consumer");
